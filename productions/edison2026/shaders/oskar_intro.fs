...
DistanceInfo oskar(in vec3 p) {
    ...
    float d1 = udRoundBox(p, vec3(1.25, 2, 1.25), 0.5);

    float d2 = sdBox(o, vec3(16.5));
    float d = opIntersection(d1, d2);
    return DistanceInfo(d1, screenType);
}
...
vec3 getColor(in MarchResult result)
{
    ...
    if (result.type == screenType) {
        ...
        color = mix(color, rust(result.position.xz * 0.01), 0.3);
...
void main()
{
    ...
    rayOrigin = vec3(0, 40, 70 - iTime*10);
    gEye = rayOrigin; // TODO is this correct?
    vec3 tar = rayOrigin + vec3(0, -1 , -1);
